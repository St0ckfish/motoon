<script>
  ...
  // تنفيذ بمجرد تحميل المكون
  onMount(() => {
    // محاولة تحديد قيمة افتراضية
    const select = document.getElementById('matn-select');
    if (select && select.options.length > 0) {
      console.log("MatnSelector mounted, initial value:", select.value);
      // إرسال حدث change اصطناعي للتحميل التلقائي
      ...
      console.error("MatnSelector: select element not found or empty");
    }
  });
  
  function handleChange(e) {
    console.log("MatnSelector: Change event triggered with value:", e.target.value);
    if (e.target && e.target.selectedOptions && e.target.selectedOptions.length > 0) {
      dispatch('change', e);
    } else {
      console.error("Invalid selection event:", e);
    }
  }
</script>
